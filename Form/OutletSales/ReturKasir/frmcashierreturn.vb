...
Public Class frmcashierreturn
    ...
    Private Sub frmcashierreturn_Activated(sender As Object, e As System.EventArgs) Handles Me.Activated
    ...
    Private Sub frmreceiptmoney_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        Call insert_no_trans("CASHIER_RETURN", Month(txt_date.Value), Year(txt_date.Value))
        Call select_control_no("CASHIER_RETURN", "TRANS")
    ...
    Public Sub insert_data()
        ...
        Dim i As Integer
        If insert = 1 Then
            Call insert_cashier_return(Trim(txt_return_no.Text), Format(txt_date.Value, "yyyy-MM-dd"), Trim(txt_cust_nm.Text), _
                                    ...
                                    0, "", "", 0, "", 0, 0, "", cbo_sales_no.Text, 0, 0, "INSERT", return_value_bef, Replace(txt_kurs.Text, ",", ""), cbo_filterwh_purch1.EditValue, txt_cust_address.Text, 0, 0)
                ...
                If DataGridView1.Item(9, i).Value = True Then
                    Call insert_cashier_return(Trim(txt_return_no.Text), Format(txt_date.Value, "yyyy-MM-dd"), Trim(txt_cust_nm.Text), _
                                    ...
                                    DataGridView1.Item(4, i).Value, DataGridView1.Item(5, i).Value, _
                                     ...
                                     "", cbo_sales_no.Text, 1, 1, "INSERT", return_value_bef, Replace(txt_kurs.Text, ",", ""), cbo_filterwh_purch1.EditValue, txt_cust_address.Text, Replace(DataGridView1.Item(7, i).Value, ",", ""), Replace(DataGridView1.Item(8, i).Value, ",", ""))
                End If
            Next
            If param_sukses = True Then
                Dim info As AlertInfo = New AlertInfo(msgtitle_save_success, msgbox_save_success)
                alertControl_success.Show(Me, info)
                update_no_trans(txt_date.Value, "CASHIER_RETURN")
                pesan = MsgBox("Apakah nota akan di print?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Konfirmasi")
                If pesan = vbYes Then
                    Dim DisplayNota As New FormReturCashier
            ...
            End If
        ElseIf edit = 1 Then
            Call update_cashier_return(Trim(txt_return_no.Text), Format(txt_date.Value, "yyyy-MM-dd"), Trim(txt_idsupplier.Text), _
                                     var_paymethod, Replace(txt_subtotal.Text, ",", ""), _
                                    Replace(txt_disc.Text, "%", ""), Replace(txt_tax.Text, "%", ""), _
                                    Replace(txt_total.Text, ",", ""), Trim(txt_comment.Text), username, Format(server_datetime(), "yyyy-MM-dd"), username, _
                                    Format(server_datetime(), "yyyy-MM-dd"), _
                                    0, "", "", 0, "", 0, 0, "", cbo_sales_no.Text, 0, 0, "UPDATE", return_value_bef, Replace(txt_kurs.Text, ",", ""), cbo_filterwh_purch1.EditValue, txt_cust_address.Text, 0, 0)
            For i = 0 To DataGridView1.Rows.Count - 1
                If DataGridView1.Item(8, i).Value = True Then
                    Call update_cashier_return(Trim(txt_return_no.Text), Format(txt_date.Value, "yyyy-MM-dd"), Trim(txt_idsupplier.Text), _
                                    ...
                                    DataGridView1.Item(4, i).Value, DataGridView1.Item(5, i).Value, _
                                     Replace(DataGridView1.Item(6, i).Value, ",", ""), _
                                     "", cbo_sales_no.Text, 1, 1, "UPDATE", return_value_bef, Replace(txt_kurs.Text, ",", ""), cbo_filterwh_purch1.EditValue, txt_cust_address.Text, Replace(DataGridView1.Item(7, i).Value, ",", ""), Replace(DataGridView1.Item(8, i).Value, ",", ""))
                End If
            Next
            If param_sukses = True Then
    ...
    Private Sub btn_del2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_del2.Click
        ...
        If edit = 1 Then
            If getTemplateAkses(username, "MN_CASHIER_RETURN_DELETE") <> True Then
        ...
        If edit = 1 Then
            ...
            If pesan = vbYes Then
                Call delete_cashier_return(Trim(txt_return_no.Text), Format(txt_date.Value, "yyyy-MM-dd"), Trim(txt_idsupplier.Text), _
                                     var_paymethod, Replace(txt_subtotal.Text, ",", ""), _
                                    Replace(txt_disc.Text, "%", ""), Replace(txt_tax.Text, "%", ""), _
                                    Replace(txt_total.Text, ",", ""), Trim(txt_comment.Text), username, Format(server_datetime(), "yyyy-MM-dd"), username, _
                                    Format(server_datetime(), "yyyy-MM-dd"), _
                                    0, "", "", 0, "", 0, 0, "", cbo_sales_no.Text, 0, 0, "DELETE", return_value_bef, txt_cust_address.Text, 0, 0)
                If param_sukses = True Then
    ...
    Private Sub list_data()
        ...
        Try
            ...
            DT = select_cashier_return(cbo_search.Text, txt_search.Text, 0, date_filter, Format(tglawal.Value, "yyyy-MM-dd"), Format(tglakhir.Value, "yyyy-MM-dd"))
            GridControl.DataSource = DT
            GridList_Customer.Columns("no_sales_return").Caption = "No Transaksi"
            GridList_Customer.Columns("no_sales_return").Width = 170
            ...
            GridList_Customer.Columns("customer_name").Width = 250
            GridList_Customer.Columns("customer_addr").Visible = False
            GridList_Customer.Columns("payment_method").Caption = "Metode Retur"
            GridList_Customer.Columns("payment_method").Width = 180
            GridList_Customer.Columns("id_warehouse").Caption = "Gudang Retur"
            GridList_Customer.Columns("id_warehouse").Width = 80
    ...
    Public Sub detail(ByVal criteria As String)
        ...
        ElseIf chk_date.Checked = False Then
            date_filter = 0
        End If
        DT = select_cashier_return("Detail", criteria, 0, date_filter, Format(tglawal.Value, "yyyy-MM-dd"), Format(tglakhir.Value, "yyyy-MM-dd")) 'ganti SP
        Rows = DT.Rows.Count - 1
        DataGridView1.Rows.Clear()
        ...
        txt_idsupplier.Text = DT.Rows(0).Item("no_sales")
        ...
        For i = 0 To Rows
            ...
            DataGridView1.Item(4, i).Value = DT.Rows(i).Item("unit")
            DataGridView1.Item(5, i).Value = FormatNumber(DT.Rows(i).Item("price"), 0)
            DataGridView1.Item(6, i).Value = FormatNumber(DT.Rows(i).Item("nominal"), 0)
            DataGridView1.Item(7, i).Value = FormatNumber(DT.Rows(i).Item("disc_nom"), 0)
            DataGridView1.Item(8, i).Value = FormatNumber(DT.Rows(i).Item("nominal_net"), 0)
    ...
    Private Sub cbo_purchase_no_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cbo_sales_no.Click
        open_conn()
        'Dim NewDisplayAcc As New frm_display_cashier
        frm_display_cashier.formsource_cashier_return = True
        frm_display_cashier.Show()
    ...
    Private Sub txt_idsupplier_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles txt_idsupplier.TextChanged
        ...
        If edit = 0 Then
            ...
            DT = select_cashier2("Detail", cbo_sales_no.Text, 0)

            Rows = DT.Rows.Count - 1
            DataGridView1.Rows.Clear()
            Dim i As Integer
            For i = 0 To Rows
                qty_max = get_qtymax_returcashier(cbo_sales_no.Text, DT.Rows(i).Item("id_item"))
                DataGridView1.Rows.Add()
                DataGridView1.Item(0, i).Value = (i + 1)
                DataGridView1.Item(1, i).Value = DT.Rows(i).Item("id_item")
                DataGridView1.Item(2, i).Value = DT.Rows(i).Item("item_name")
                DataGridView1.Item(3, i).Value = FormatNumber(qty_max, 0)
                DataGridView1.Item(4, i).Value = DT.Rows(i).Item("id_unit")
                DataGridView1.Item(5, i).Value = FormatNumber(DT.Rows(i).Item("price"), 0)
                DataGridView1.Item(6, i).Value = FormatNumber(qty_max * DT.Rows(i).Item("price"), 0)
                DataGridView1.Item(7, i).Value = FormatNumber(DT.Rows(i).Item("disc_nom"), 0)
                DataGridView1.Item(8, i).Value = FormatNumber(DataGridView1.Item(6, i).Value - (DataGridView1.Item(6, i).Value * DT.Rows(i).Item("disc_nom") / 100), 0)
                'DataGridView1.Item(8, i).Value = DT.Rows(i).Item("no_sales")
            Next
            If edit = 1 Then
            ...
            End If
            If qty_max = 0 Then
                Dim info As AlertInfo = New AlertInfo("Limit Data", "Tidak ada qty yang dapat di retur")
                alertControl_warning.Show(Me, info)
                clean()
                Exit Sub
            End If
    ...
    Private Sub btn_save2_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_save2.Click
        ...
        If Trim(cbo_paymethod.Text) = "" Then
            Dim info As AlertInfo = New AlertInfo("Cek Kevaliditasan Data", "Masukkan cara pembayaran")
            alertControl_warning.Show(Me, info)
            Exit Sub
        End If
        ...
        Next
        If b = 0 Then
            Dim info As AlertInfo = New AlertInfo("Cek Kevaliditasan Data", "Item belum diisi")
            alertControl_warning.Show(Me, info)
            Exit Sub
        ElseIf a > 0 Then
            insert_data()
    ...
    Private Sub DataGridView1_CellBeginEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellCancelEventArgs) Handles DataGridView1.CellBeginEdit
        ...
        If start = 0 Then
            If edit = 1 Then
                qty_max = DataGridView1.Item(3, DataGridView1.CurrentCell.RowIndex).Value + get_qtymax_returcashier(cbo_sales_no.Text, DataGridView1.Item(1, DataGridView1.CurrentCell.RowIndex).Value)
            End If

            If insert = 1 Then
                qty_max = get_qtymax_returcashier(cbo_sales_no.Text, DataGridView1.Item(1, DataGridView1.CurrentCell.RowIndex).Value)
                If qty_max = 0 Then
                    qty_max = DataGridView1.Item(3, DataGridView1.CurrentCell.RowIndex).Value
                End If
            End If
    ...
    Private Sub DataGridView1_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridView1.CellEndEdit
        ...
        If qty_max < DataGridView1.Item(3, DataGridView1.CurrentCell.RowIndex).Value Then
            Dim info As AlertInfo = New AlertInfo("Cek Kevaliditasan Data", "Qty Retur tidak boleh lebih besar" & vbCrLf & " dari qty faktur")
            alertControl_warning.Show(Me, info)
            DataGridView1.Item(3, DataGridView1.CurrentCell.RowIndex).Value = qty_max
        End If

        If columnIndex = 3 Then
            DataGridView1.Item(6, DataGridView1.CurrentCell.RowIndex).Value = FormatNumber(DataGridView1.Item(5, DataGridView1.CurrentCell.RowIndex).Value * DataGridView1.Item(3, DataGridView1.CurrentCell.RowIndex).Value, 0)
            DataGridView1.Item(8, DataGridView1.CurrentCell.RowIndex).Value = FormatNumber(DataGridView1.Item(6, DataGridView1.CurrentCell.RowIndex).Value - (DataGridView1.Item(7, DataGridView1.CurrentCell.RowIndex).Value / 100 * DataGridView1.Item(6, DataGridView1.CurrentCell.RowIndex).Value), 0)
        End If
        rows = DataGridView1.Rows.Count - 1
        Dim i As Integer
        For i = 0 To rows
